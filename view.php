...
<head>
    ...
    <style>
        body { user-select: none; -webkit-user-select: none; background: #080A0C; color: #F5F3EE; }
        .viewer-shell { padding: 0; overflow: hidden; background: linear-gradient(135deg, #0D1013, #111519); border: 1px solid rgba(255,255,255,0.08); }
        .viewer-toolbar { display: flex; justify-content: space-between; align-items: center; padding: 14px 18px; border-bottom: 1px solid rgba(255,255,255,0.08); background: rgba(10,12,14,0.82); }
        .viewer-toolbar__meta strong { color: #F5F3EE; }
        .viewer-toolbar__meta span { color: #C4C3BE; }
        .book-reader-stage { display: flex; justify-content: center; align-items: center; padding: 24px; background: radial-gradient(circle at top, rgba(200,166,106,0.10), transparent 30%), linear-gradient(135deg, #0D1013, #12161A); min-height: 78vh; }
        .book-reader-page { position: relative; width: min(100%, 940px); border-radius: 20px; border: 1px solid rgba(255,255,255,0.12); box-shadow: 0 30px 80px rgba(0,0,0,0.35); overflow: hidden; background: #fff; min-height: 72vh; }
        .book-reader-page iframe { width: 100%; height: 72vh; min-height: 72vh; border: 0; background: #fff; }
        .book-reader-page::before { content: ''; position: absolute; inset: 0; pointer-events: none; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.04), inset 0 0 30px rgba(0,0,0,0.08); }
        .reader-note { margin: 0; font-size: 0.94rem; color: #C4C3BE; padding: 0 24px 20px; background: #0D1013; }
        .page-indicator { display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 999px; background: rgba(255,255,255,0.04); color: #F5F3EE; font-weight: 600; border: 1px solid rgba(255,255,255,0.10); }
        .button--secondary { background: rgba(255,255,255,0.06); color: #F5F3EE; border: 1px solid rgba(255,255,255,0.12); }
        .button--secondary:hover { background: rgba(255,255,255,0.10); }
        @media (max-width: 900px) { .book-reader-stage { padding: 12px; } .book-reader-page { width: 100%; } .book-reader-page iframe { height: 70vh; min-height: 70vh; } }
    </style>
</head>
<body>
<div class="container">
    <header class="site-header">
        <a class="site-brand" href="index.php">
            ...
            <div>
                ...
                <div style="font-size:0.95rem;color:#6b7280;">Read the PDF directly in your browser.</div>
            </div>
        </a>
        <nav class="nav-links">
        ...
        <div class="viewer-toolbar">
            ...
            <div class="viewer-toolbar__actions">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <span id="page-display" style="min-width:80px;text-align:center;color:#cdb89e;">Page 1 / 1</span>
                    <button id="next-page" class="button button--secondary">Next ▶</button>
                    <a class="button button--secondary" href="index.php">Back to library</a>
                </div>
            </div>
        </div>
        <div class="book-reader-stage">
            <div class="book-reader-page">
                <div id="pdf-viewer" style="width:100%;height:72vh;min-height:72vh;display:flex;align-items:center;justify-content:center;position:relative;overflow:auto;padding:8px;box-sizing:border-box;background:#fff;border-radius:8px;">
                    <canvas id="pdf-canvas" style="max-width:100%;max-height:100%;display:block;border-radius:8px;background:#fff;"></canvas>
                    <img id="pdf-image" style="display:none;max-width:100%;max-height:100%;object-fit:contain;border-radius:8px;background:#fff;" alt="">
                </div>
            </div>
        </div>
